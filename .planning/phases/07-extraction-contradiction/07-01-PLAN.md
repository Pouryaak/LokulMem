...
must_haves:
  truths:
    ...
    - "Extraction threshold filters low-quality facts (default 0.45)"
...
- **EXTRACT-05**: Extraction threshold default 0.45 (configurable)
...
export interface ExtractionConfig {
  /** Default extraction threshold (0-1) - default: 0.45 */
  threshold: number;
...
export class RecurrenceTracker {
  ...
  track(content: string, embedding: Float32Array): void {
    // Generate simple hash from content (use crypto.subtle in real implementation)
...
5. **Extraction threshold default 0.45** configurable with type-specific overrides
...
**Issue 1: Entity types incomplete**
- Fix: Added documentation that 'possession' is NOT a separate entity type. Possessions are tracked via a flag on memories, not as entities. This prevents Entity.type union violation.

**Issue 2: Memory type fallback is wrong**
- Fix: Changed `memoryTypes: memoryTypes.length > 0 ? memoryTypes : ['preference']` to `memoryTypes: memoryTypes.length ? [...new Set(memoryTypes)] : []`. Empty array lets QualityScorer use base threshold as fallback instead of poisoning contradiction domains.

**Issue 3: RecurrenceTracker keying by full content string is risky**
- Fix: Store embeddings keyed by content hash (via simpleHash()), not full content string. This prevents paraphrases from being treated as different and avoids large texts as keys.
