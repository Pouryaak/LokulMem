...
**Issue 1: Character positions contradict earlier decision**
- Fix: Removed `position` field from TemporalMarker interface. Per Phase 7 CONTEXT decision: "Position tracking: No - don't store character offsets." Position is debug-only and never persisted.

**Issue 2: Pattern list too broad**
- Fix: Removed `\bold\b` pattern which matched "old laptop", "old code" causing false positives. Replaced with comment noting specific pattern should be used if needed: `\bold (?:address|job|company|role|city|house|apartment)\b`.
